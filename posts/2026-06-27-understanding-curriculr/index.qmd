...
subtitle: "Turning a spreadsheet into a typeset CV"
description: "A look at how curriculr separates CV content from layout, letting a single Excel workbook drive a polished, Typst-rendered document through Quarto."
abstract: >
  curriculr is an R package for producing data-driven curricula vitae from a
  structured Excel workbook rather than hand-edited markup. I walk through the
  core workflow -- reading CV content from the workbook, converting it into
  Typst layout blocks, and rendering the result as a PDF via Quarto -- and
  explain the design choice behind it: content lives in the spreadsheet,
  rendering configuration lives in Quarto, and transformation logic lives in
  small, reusable R functions. I also cover the resume/CV variant switch, which
  lets you toggle a shorter version of the same document without touching any
  R code, and situate curriculr within the broader reproducible-research
  ecosystem it shares conventions with (toolero, containr, and submitr).
author:
- name: Erwin Lares
...
![Caption](path/to/image.jpg){#fig-id width="500px" height="375px" fig-align="center" fig-alt="Descriptive text for screen readers"}
